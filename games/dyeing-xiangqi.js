...
class DyeingXiangqiRoom extends QiTwoPlayerRoomBase {
    ...
    _startClockTicker() {
        ...
        this._clockInterval = setInterval(() => {
            ...
            if (lostSlot) {
                ...
                this.recordResultText = lostSlot === 'player1' ? '红超时绿胜' : '绿超时红胜';
    ...
    exportRecord() {
        return {
            ...
            moves: this.moveHistory.map((m) => (
                `${(m.player === 'player1' ? 'R' : 'G')}${m.fromRow},${m.fromCol}-${m.toRow},${m.toCol}`
            )),
    ...
    importRecord(data, requesterWs) {
        ...
        for (let i = 0; i < rawMoves.length; i++) {
            ...
            if (typeof entry === 'string') {
                const m = entry.match(/^([RG])(\d+),(\d+)-(\d+),(\d+)$/i);
                if (!m) {
                    this.resetToEmpty();
        ...
        if (data.result && !this.gameOver) {
            ...
            else if (data.result === 'player2' || /绿胜/.test(rt)) this.winner = 'player2';
    ...
    /** 聊天里显示的执方名：本棋种是「红方 / 绿方」，不是黑/白 */
    getChatSideLabel(slot) {
        return slot === 'player1' ? '红方' : (slot === 'player2' ? '绿方' : String(slot));
    }
        ...
        switch (msg.type) {
            ...
            case 'resign':
                ...
                if (this.gameOver && slot) {
                    this.recordResultText = slot === 'player1' ? '红认输绿胜' : '绿认输红胜';
                    this._stopClockTicker();
                }
                break;
            case 'requestNewGame':
