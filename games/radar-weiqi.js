...
class InvisibleStone2WeiqiRoom extends QiTwoPlayerRoomBase {
    ...
    buildInvisibleTintFromState(board, inv, slot, gameOver) {
        ...
        for (let r = 0; r < this.boardSize; r++) {
            for (let c = 0; c < this.boardSize; c++) {
                ...
                if (!inv[r][c]) continue;
                if (slot === 'player1' && board[r][c] === 1) {
                    list.push({ row: r, col: c });
                } else if (slot === 'player2' && board[r][c] === 2) {
                    list.push({ row: r, col: c });
                }
            }
        }
    ...
    /**
     * 在服务端用完整局面计算：对每个「本座位可见」的棋子，到最近一枚对方棋子（含对方隐身子）的曼哈顿距离。
     * 只下发距离网格，不下发对方隐身子坐标。
     */
    buildOpponentDistanceGridFromState(board, inv, slot, gameOver) {
        const viewBoard = this.buildViewBoardFromState(board, inv, slot, gameOver);
        const out = Array(this.boardSize).fill().map(() => Array(this.boardSize).fill(null));
        for (let r = 0; r < this.boardSize; r++) {
            for (let c = 0; c < this.boardSize; c++) {
                if (viewBoard[r][c] === 0) continue;
                const my = viewBoard[r][c];
                const opp = 3 - my;
                let best = Infinity;
                for (let r2 = 0; r2 < this.boardSize; r2++) {
                    for (let c2 = 0; c2 < this.boardSize; c2++) {
                        if (board[r2][c2] !== opp) continue;
                        const d = Math.abs(r - r2) + Math.abs(c - c2);
                        if (d < best) best = d;
                    }
                }
                out[r][c] = best === Infinity ? null : best;
            }
        }
        return out;
    }

    buildOpponentDistanceGridForSlot(slot) {
        return this.buildOpponentDistanceGridFromState(this.board, this.invisible, slot, this.gameOver);
    }

    buildReplayOpponentDistanceGridsForSlot(slot) {
        const gameOver = this.gameOver;
        const out = [];
        let board = this.copyBoard(this.openingBoard);
        let inv = this.emptyInvisible();
        out.push(this.buildOpponentDistanceGridFromState(board, inv, slot, gameOver));
        for (let i = 0; i < this.historyBoards.length; i++) {
            board = this.copyBoard(this.historyBoards[i]);
            inv = this.copyInvisible(this.historyInvisible[i]);
            out.push(this.buildOpponentDistanceGridFromState(board, inv, slot, gameOver));
    ...
    getStateForClient(ws) {
        ...
        return {
            ...
            opponentDistanceGrid: this.buildOpponentDistanceGridForSlot(slot),
            replayOpponentDistanceGrids: this.buildReplayOpponentDistanceGridsForSlot(slot),
    ...
    resetGame() {
        ...
        for (const client of allClients) {
            if (client.readyState !== 1) continue;
            client.send(JSON.stringify({ type: 'newGameStarted', ...this.getStateForClient(client), slots: { player1: false, player2: false } }));
        }
    ...
    importRecord(data, requesterWs) {
        if (!data || (data.gameId !== 'radar-weiqi')) {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要雷达围棋棋谱）。' }));
            return;
        }
        ...
        const moves = rawMoves.map(InvisibleStone2WeiqiRoom.parseMove);
        ...
        for (let i = 0; i < moves.length; i++) {
            ...
            if (move.type === 'move') {
                ...
                const wantInv = (pwsh == null || importHand < pwsh);
        ...
        if (data.result || data.resultText) {
            ...
            this.winner = InvisibleStone2WeiqiRoom.parseResultTextToWinner(importedResultText);
...
module.exports = {
    initRoom(room) {
        room.gameLogic = new InvisibleStone2WeiqiRoom(room);
