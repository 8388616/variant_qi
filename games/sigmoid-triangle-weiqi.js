...
const { QiTwoPlayerRoomBase, qiProtocol, qiMatchTimeControl, gridGraphWeiqiRules, applyInitialPositionCompact, encodeInitialPositionCompact, qiBoardSeatOverlay } = require('../common');

/** 方格棋盘上：四向 + 右上、左下（中腹最多 6 气） */
class SigmoidTriangleWeiqiRoom extends QiTwoPlayerRoomBase
{
    static getNeighbors(row, col, boardSize) {
        const out = [];
        const push = (r, c) => {
    ...
    getNeighbors(r, c) {
        return SigmoidTriangleWeiqiRoom.getNeighbors(r, c, this.boardSize);
    }

    _nb() {
        return (r, c) => this.getNeighbors(r, c);
    }
    constructor(room, initialSize = 13) {
        super(room);
    ...
    isLibertySurroundedByOpponent(board, libertyRow, libertyCol, opponentColor) {
        return gridGraphWeiqiRules.isLibertySurroundedByOpponent(
            board, libertyRow, libertyCol, opponentColor, this._nb()
        );
    }
    ...
    computeScore(liveBoard, territory) {
        return gridGraphWeiqiRules.computeScore(
            liveBoard, territory, this.boardSize, this.boardSize, () => true
        );
    }
    ...
    handleMessage(ws, msg)
    {
        ...
        switch (msg.type)
        {
            ...
            // editBoard 由 qiBoardSeatOverlay → installStandardEditBoard 统一处理
    ...
    exportRecord()
    {
        ...
        return {
            format: 'muzei',
            version: 1,
            gameType: 'Sigmoid三角围棋',
            gameId: 'sigmoid-triangle-weiqi',
            boardSize: this.boardSize,
            komi: this.boardSize <= 8 ? 4.25 : 3.25,
    ...
    importRecord(data, requesterWs) {
        if (!data || data.gameId !== 'sigmoid-triangle-weiqi') {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要 Sigmoid 三角围棋棋谱）。' }));
            return;
        }
        const newSize = data.boardSize || 13;
        if (!Number.isInteger(newSize) || newSize > 21) {
        ...
        const moves = rawMoves.map(SigmoidTriangleWeiqiRoom.parseMove);
        ...
        if (data.result || data.resultText) {
            ...
            this.winner = SigmoidTriangleWeiqiRoom.parseResultTextToWinner(importedResultText);
...
module.exports = {
    initRoom(room) {
        room.gameLogic = new SigmoidTriangleWeiqiRoom(room);
