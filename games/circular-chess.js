    ...
    applyEditBoard(ws, msg) {
        ...
        for (let i = 0; i < edited.length; i++) {
            ...
            if (v !== '') {
                ...
                if (v[1] === 'p') {
                    next[R.key(ring, sector)] = { 0: v[0], 1: 'p', dir: R.pawnDir(v[0] === 'w' ? 'player1' : 'player2', sector), steps: 0 };
                } else {
    ...
    exportRecord() {
        return {
            ...
            moves: this.moveHistory.map((m) => {
                let s = `${(m.player === 'player1' ? 'W' : 'B')}${m.fromRing},${m.fromSector}-${m.toRing},${m.toSector}`;
