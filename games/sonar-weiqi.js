...
class SonarWeiqiRoom extends QiTwoPlayerRoomBase {
    ...
    /**
     * 沿某一方向扫描，返回最近一枚对方棋子坐标（含对方隐身子）；该方向无对方子则为 null。
     */
    findNearestOpponentInDirection(board, row, col, dr, dc, opp) {
        let r = row + dr;
        let c = col + dc;
        while (r >= 0 && r < this.boardSize && c >= 0 && c < this.boardSize) {
            if (board[r][c] === opp) return { row: r, col: c };
            r += dr;
            c += dc;
        }
        return null;
    ...
    /**
     * 声呐：对本座位每枚己方棋子，在上下左右四个方向上找到最近的对方棋子并显示出来（仅视野，不永久显形）。
     */
    applySonarRevealToView(out, board, slot) {
        if (slot !== 'player1' && slot !== 'player2') return;
        const my = slot === 'player1' ? 1 : 2;
        const opp = 3 - my;
        const dirs = [[-1, 0], [1, 0], [0, -1], [0, 1]];
        for (let r = 0; r < this.boardSize; r++) {
            for (let c = 0; c < this.boardSize; c++) {
                if (out[r][c] !== my) continue;
                for (const [dr, dc] of dirs) {
                    const hit = this.findNearestOpponentInDirection(board, r, c, dr, dc, opp);
                    if (hit) out[hit.row][hit.col] = opp;
                }
            }
        }
    }
    ...
    buildViewBoardFromState(board, inv, slot, gameOver) {
        ...
        this.applySonarRevealToView(out, board, slot);
        return out;
    }
    ...
    /** 最后一手标记：仅当本座位视野中看得到该子时显示（含声呐显视的对方隐身子） */
    filterLastMoveMarkers(slot) {
        ...
        const view = this.buildViewBoard(slot);
    ...
    importRecord(data, requesterWs) {
        if (!data || (data.gameId !== 'sonar-weiqi')) {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要声呐围棋棋谱）。' }));
            return;
        }
        ...
        const moves = rawMoves.map(SonarWeiqiRoom.parseMove);
        ...
        if (data.result || data.resultText) {
            ...
            this.winner = SonarWeiqiRoom.parseResultTextToWinner(importedResultText);
...
module.exports = {
    initRoom(room) {
        room.gameLogic = new SonarWeiqiRoom(room);
