...
class InstabilityWeiqiRoom extends QiTwoPlayerRoomBase
{
    ...
    performUndo(steps, requesterWs) {
        ...
        if (this.historySnapshots.length === 0) {
            ...
            if (this.moveCoords.length > 0) {
                ...
                if (last.type === 'move')
                    this.lastMoveMarkers = [{ row: last.row, col: last.col, color: last.player === 'player1' ? 1 : 2 }];
                else
                    this.lastMoveMarkers = [];
            } else
                this.lastMoveMarkers = [];
    ...
    /** 棋谱 initialPosition：优先紧凑字符串数组 ["B3,3","W4,4"]；兼容旧版 { player1:[], player2:[] }。初始棋子均为稳定子（unstableInfo 为 0）。 */
