...
class DuoQuadricolourRoom extends QiTwoPlayerRoomBase
{
    ...
    _slotOfTurn() {
        return sideOfColor(this.turnColor) === 1 ? 'black' : 'white';
    }
    ...
    onResignResolved(resignSlot) {
        ...
        this.recordResultText = resignSlot === 'black' ? '白红方中盘胜' : '黑蓝方中盘胜';
    }
    ...
    setTimeLossResultText(lostSlot) {
        if (lostSlot === 'black') this.recordResultText = '黑蓝方超时白红方胜';
        else if (lostSlot === 'white') this.recordResultText = '白红方超时黑蓝方胜';
    }
    ...
    static parseResultTextToWinner(resultText) {
        ...
        if (resultText.includes('黑蓝方胜')) return 'black';
        if (resultText.includes('白红方胜')) return 'white';
    ...
    exportRecord()
    {
        ...
        if (this.gameOver) {
            ...
            else if (this.winner === 'black') resultText = '黑蓝方胜';
            else if (this.winner === 'white') resultText = '白红方胜';
        }
    ...
    static parseMove(entry) {
        if (typeof entry === 'string') {
            ...
            const slot = sideOfColor(color) === 1 ? 'black' : 'white';
