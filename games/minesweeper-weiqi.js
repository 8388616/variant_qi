...
class MinesweeperWeiqiRoom extends QiTwoPlayerRoomBase
{
    ...
    onResignResolved(resignSlot) { this.recordResultText = resignSlot === 'player1' ? '白中盘胜' : '黑中盘胜'; this.minesRevealedPublicly = true; }
    onDrawResolved() { this.recordResultText = '和棋'; this.minesRevealedPublicly = true; }
    ...
    assignTerritoryWithRange(liveBoard) {
        const isHole = (r, c) => liveBoard[r][c] === MINE;
        return squareWeiqiRules.assignTerritoryWithRangeWithHoles(liveBoard, this.boardSize, isHole);
    }
    ...
    /** 棋盘上尚未被踩掉的雷数（客户端棋盘可能 scrub 掉雷，故由服务端下发） */
    countRemainingMines() {
        let n = 0;
        for (let r = 0; r < this.boardSize; r++) {
            for (let c = 0; c < this.boardSize; c++) {
                if (this.board[r][c] === MINE) n++;
            }
        }
        return n;
    }
    ...
    getState() {
        return {
            ...
            remainingMines: this.holesGenerated ? this.countRemainingMines() : null,
    ...
    handleMessage(ws, msg) {
        ...
        switch (msg.type) {
            ...
            case 'editBoard':
                // 锁定由 installStandardEditBoard / isBoardEditLocked 先行拦截；此处再防一层
                if (typeof qiProtocol.isBoardEditLocked === 'function'
                    ? qiProtocol.isBoardEditLocked(this)
                    : (this.gameOver || this.matchStarted || this.tcSettings
                        || (this.moveCoords && this.moveCoords.length > 0)
                        || this.historyBoards.length > 1)) {
