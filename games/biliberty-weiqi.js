...
class BilibertyWeiqiRoom extends QiTwoPlayerRoomBase
{
    ...
    tryPlaceStone(boardBefore, row, col, playerVal) {
        return squareWeiqiRules.tryPlaceStoneNLiberty(
            boardBefore, row, col, playerVal, this.boardSize, (b) => this.copyBoard(b), 2
        );
    }
    ...
    importRecord(data, requesterWs) {
        if (!data || data.gameId !== 'biliberty-weiqi') {
        ...
        if (data.result || data.resultText) {
            ...
            this.winner = BilibertyWeiqiRoom.parseResultTextToWinner(importedResultText);
