...
class GreedyWeiqiRoom extends QiTwoPlayerRoomBase
{
    ...
    _countOpponentCaptures(boardBefore, boardAfter, playerVal) {
    ...
    /**
     * 是否存在比 myCap 提子更多的合法点（含禁全同过滤）。
     * myCap=0 时用于判定「是否必须提子」。
     */
    _hasStrictlyBetterCapture(boardBefore, playerVal, myCap, skipRow = -1, skipCol = -1) {
        for (let r = 0; r < this.boardSize; r++) {
            for (let c = 0; c < this.boardSize; c++) {
                if (boardBefore[r][c] !== 0) continue;
                if (r === skipRow && c === skipCol) continue;
                const nb = this._basePlaceStone(boardBefore, r, c, playerVal);
                if (!nb) continue;
                if (this.historyBoardSet.has(this.boardToString(nb))) continue;
                if (this._countOpponentCaptures(boardBefore, nb, playerVal) > myCap) return true;
            }
        }
        return false;
    }

    maxLegalCaptureCount(boardBefore, playerVal) {
        let maxCap = 0;
        for (let r = 0; r < this.boardSize; r++) {
            for (let c = 0; c < this.boardSize; c++) {
                if (boardBefore[r][c] !== 0) continue;
                ...
                if (!nb) continue;
                if (this.historyBoardSet.has(this.boardToString(nb))) continue;
                const cap = this._countOpponentCaptures(boardBefore, nb, playerVal);
    ...
    tryPlaceStone(boardBefore, row, col, playerVal) {
        const newBoard = this._basePlaceStone(boardBefore, row, col, playerVal);
        if (!newBoard) return null;
        const myCap = this._countOpponentCaptures(boardBefore, newBoard, playerVal);
        if (this._hasStrictlyBetterCapture(boardBefore, playerVal, myCap, row, col)) return null;
        return newBoard;
    }
    ...
    computeLead()
    {
        // Benson 加成：保活无条件活棋链 + 确定领地覆盖（其余保持原启发式）
    ...
    handleMessage(ws, msg)
    {
        ...
        switch (msg.type)
        {
            ...
            case 'move': {
                ...
                if (!moveSlot || moveSlot !== (this.currentPlayer === 1 ? 'player1' : 'player2')) break;
                const { row, col } = msg;
                if (Number.isInteger(row) && Number.isInteger(col)
                    && row >= 0 && row < this.boardSize && col >= 0 && col < this.boardSize
                    && this.board[row][col] === 0) {
                    const playerVal = this.currentPlayer === 1 ? 1 : 2;
                    const base = this._basePlaceStone(this.board, row, col, playerVal);
                    if (base) {
    ...
    importRecord(data, requesterWs) {
        if (!data || data.gameId !== 'greedy-weiqi') {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要贪吃围棋棋谱）。' }));
            return;
        }
        ...
        const moves = rawMoves.map(GreedyWeiqiRoom.parseMove);
        ...
        if (data.result || data.resultText) {
            ...
            this.winner = GreedyWeiqiRoom.parseResultTextToWinner(importedResultText);
