...
class RealTimeWeiqiRoom extends QiTwoPlayerRoomBase {
    ...
    constructor(room, initialSize = 19) {
        ...
        /** 共用棋钟已耗尽后进入数点；拒绝数点结果时不恢复对弈，仅再次提议数点 */
    ...
    _realtimePlaceStone(ws, msg, moveSlot) {
        ...
        if (this.pendingScore) {
            ws.send(JSON.stringify({ type: 'error', message: '正在数点确认中，无法落子。' }));
            return;
        }
        const now = Date.now();
        if (!this._drainSharedBeforeAction(now)) {
            ws.send(JSON.stringify({ type: 'error', message: '共用棋钟已用尽，已进入数点。' }));
            return;
        }
        ...
        const playerVal = moveSlot === 'player1' ? 1 : 2;
    ...
    importRecord(data, requesterWs) {
        if (!data || data.gameId !== 'real-time-weiqi') {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要RealTime围棋棋谱）。' }));
            return;
        }
