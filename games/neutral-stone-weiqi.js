...
class NeutralStoneWeiqiRoom extends QiTwoPlayerRoomBase
{
    constructor(room, initialSize = 19) {
        super(room);
        this.editBoardAllowedValues = [0, 1, 2, NEUTRAL];
    ...
    tryPlaceStone(boardBefore, row, col, playerVal)
    {
        ...
        for (let [dr, dc] of dirs)
        {
            ...
            if ((v === NEUTRAL || v === 3 - playerVal) && !this.hasLiberty(newBoard, nr, nc))
                toRemove.push({ row: nr, col: nc, color: v });
        }
        for (const stone of toRemove)
            if (newBoard[stone.row][stone.col] === stone.color)
                this.removeGroup(newBoard, stone.row, stone.col, stone.color);
