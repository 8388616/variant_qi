...
struct GTPEngine {
  ...
  const bool configSnubQuadrangleBoard;
  ...
  GTPEngine(
    ...
    bool snubQuadrangleBoardFromConfig,
    ...
    :nnModelFile(modelFile),
     ...
     configSnubQuadrangleBoard(snubQuadrangleBoardFromConfig),
  ...
  void setOrResetBoardSize(ConfigParser& cfg, Logger& logger, Rand& seedRand, int boardXSize, int boardYSize, bool loggingToStderr) {
    ...
    const bool wantSnub =
      configSnubQuadrangleBoard && boardXSize == boardYSize && SnubQuadrangle::isSnubGridLen(boardXSize);

    //If the bot is wrongly sized, we need to create or recreate the bot
    if(bot == NULL ||
       bot->getRootBoard().x_size != boardXSize ||
       bot->getRootBoard().y_size != boardYSize ||
       (bot->getRootBoard().isSnubQuadrangle != wantSnub)) {
      if(bot != NULL) {
        assert(bot != NULL);
        bot->stopAndWait();
        delete bot;
      ...
      if(wantSnub)
        board.initSnubQuadrangleBoard(SnubQuadrangle::lanesFromGridLen(boardXSize));
      Player pla = P_BLACK;
  ...
  void clearBoard() {
    ...
    if(bot->getRootBoard().isSnubQuadrangle)
      board.initSnubQuadrangleBoard(SnubQuadrangle::lanesFromGridLen(newXSize));
    Player pla = P_BLACK;
    BoardHistory hist(board,pla,currentRules,0);
    vector<Move> newMoveHistory;
    setPositionAndRules(pla,board,hist,board,pla,newMoveHistory);
...
int MainCmds::gtp(const vector<string>& args) {
  ...
  const bool snubQuadrangleBoardFromConfig =
    cfg.contains("snubQuadrangleBoard") && cfg.getBool("snubQuadrangleBoard");
      ...
      if(pieces.size() % 2 != 0) {
        ...
        for(int i = 0; i<pieces.size(); i += 2) {
          ...
          if(!PlayerIO::tryParsePlayer(pieces[i],pla)) {
            // 兼容 -1（网站端把异形盘的无效掩码格作为 -1 发送，表示该格无子）
            if(pieces[i] == "-1") {
              continue;
            }
            responseIsError = true;
