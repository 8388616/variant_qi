...
int MainCmds::match(const vector<string>& args) {
  ...
  try {
    ...
    TCLAP::ValueArg<string> tdataOutputDirArg("","tdata-output-dir","Dir to output training tdata npz (requires writeTdata=true in config)",false,string(),"DIR");

    cmd.add(logFileArg);
    cmd.add(sgfOutputDirArg);
    cmd.add(tdataOutputDirArg);
    ...
    tdataOutputDir = tdataOutputDirArg.getValue();
  ...
  int dataBoardLen = 19;
  int inputsVersion = NNModelVersion::getInputsVersion(NNModelVersion::defaultModelVersion);
  int maxRowsPerTrainFile = 75000;
  double firstFileRandMinProp = 0.15;
  if(playSettings.recordTrainingData) {
    // Read before warnUnusedKeys so inject/overrides for match tdata are not reported unused (same as selfplay.cpp).
    dataBoardLen = cfg.getInt("dataBoardLen",3,Board::MAX_LEN);
    if(cfg.contains("inputsVersion"))
      inputsVersion = cfg.getInt("inputsVersion",0,10000);
    maxRowsPerTrainFile = cfg.getInt("maxRowsPerTrainFile",1,100000000);
    firstFileRandMinProp = cfg.getDouble("firstFileRandMinProp",0.0,1.0);
    if(cfg.contains("writeTdata"))
      cfg.markKeyUsed("writeTdata");
    cfg.markKeyUsed("dataBoardLen");
    if(cfg.contains("inputsVersion"))
      cfg.markKeyUsed("inputsVersion");
    if(cfg.contains("maxRowsPerTrainFile"))
      cfg.markKeyUsed("maxRowsPerTrainFile");
    if(cfg.contains("firstFileRandMinProp"))
      cfg.markKeyUsed("firstFileRandMinProp");
    if(tdataOutputDir == "" && cfg.contains("tdataOutputDir"))
      tdataOutputDir = cfg.getString("tdataOutputDir");
    if(tdataOutputDir == "")
      throw StringError("writeTdata=true but no output directory (pass -tdata-output-dir or set tdataOutputDir in config)");
    if(playSettings.allowResignation)
      throw StringError("writeTdata=true is incompatible with allowResignation=true (resigned games cannot be written as training data)");
  }
  ...
  std::mutex tdataWriterMutex;
  if(playSettings.recordTrainingData) {
    MakeDir::make(tdataOutputDir);
    Rand tdataRand;
    tdataWriter = new TrainingDataWriter(
      tdataOutputDir, inputsVersion, maxRowsPerTrainFile, firstFileRandMinProp, dataBoardLen, dataBoardLen,
      Global::uint64ToHexString(tdataRand.nextUInt64())
    );
    logger.write("Writing match training tdata to: " + tdataOutputDir);
    if(!logger.isLoggingToStdout())
      cout << "Writing match training tdata to: " << tdataOutputDir << endl;
  }
  ...
  auto runMatchLoop = [
    ...
    while(true) {
      ...
      if(gameData != NULL) {
        ...
        if(tdataWriter != NULL) {
          try {
            std::lock_guard<std::mutex> lock(tdataWriterMutex);
            tdataWriter->writeGame(*gameData);
          }
          catch(const StringError& e) {
            logger.write(string("WARNING: Failed to write match training data: ") + e.what());
          }
        }
