...
int MainCmds::selfplay(const vector<string>& args) {
  ...
  // Read here so cfg.warnUnusedKeys() counts this key as used. Previously it was only read inside
  // loadLatestNeuralNetIntoManager after a successful new-model load; early returns or an older
  // binary would leave the key "unused" and ignore the setting.
  const bool appendSgfGames =
    !cfg.contains("selfplayAppendSgfGames") ? true : cfg.getBool("selfplayAppendSgfGames");
  ...
  auto loadLatestNeuralNetIntoManager =
    [inputsVersion,&manager,maxRowsPerTrainFile,firstFileRandMinProp,dataBoardLen,
     ...
     appendSgfGames](const string* lastNetName) -> bool {
    ...
    if(appendSgfGames && sgfOutputDir.length() > 0) {
      sgfOut = new ofstream();
      FileUtils::open(*sgfOut, sgfOutputDir + "/" + Global::uint64ToHexString(rand.nextUInt64()) + ".sgfs");
    }
    else if(!appendSgfGames) {
      logger.write("Note: selfplayAppendSgfGames=false - not writing appended .sgfs (tdata only).");
    }
  ...
  //Unblock data-write thread(s) that may be waiting on an empty queue (readOnly => waitPop returns).
  manager->markGameQueuesReadOnly();
