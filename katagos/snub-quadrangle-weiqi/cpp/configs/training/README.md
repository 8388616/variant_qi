...
`match.cfg` is used only by `scripts/training_loop.ps1` for `katago match` (two exported nets). It is separate from `selfplay.cfg` (self-play writes tdata; match does not). When you pass `-SelfplayOverridesPath` (typically `cpp/configs/training/overrides_fast.cfg`), the same file is also appended to the match `-config` chain so cheap search and visit tuning match self-play.
