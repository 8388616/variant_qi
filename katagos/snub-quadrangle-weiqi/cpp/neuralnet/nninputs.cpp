...
static void copySnubWithSymmetry(
  const float* src, float* dst, int nSize, int hSize, int wSize, int cSize, bool useNHWC, int symmetry, bool reverse, int snubGridLen
) {
  testAssert((symmetry & 0x4) == 0);
  testAssert(hSize == wSize);
  if(snubGridLen <= 0 || snubGridLen > hSize)
    snubGridLen = hSize;
  const int area = hSize * wSize;

  if(symmetry == 0) {
    const size_t floatCount = (size_t)nSize * (useNHWC ? (size_t)area * (size_t)cSize : (size_t)area * (size_t)cSize);
    std::memcpy(dst, src, floatCount * sizeof(float));
    return;
  }

  const uint16_t* perm = SnubQuadrangle::getNNTensorPosPermutation(hSize, snubGridLen, symmetry);
  testAssert(perm != NULL);

  if(useNHWC) {
    const int nStride = area * cSize;
    for(int n = 0; n < nSize; n++) {
      const float* srcN = src + n * nStride;
      float* dstN = dst + n * nStride;
      for(int pos = 0; pos < area; pos++) {
        const int symPos = perm[pos];
        const int srcBase = pos * cSize;
        const int dstBase = symPos * cSize;
        if(reverse) {
          for(int c = 0; c < cSize; c++)
            dstN[srcBase + c] = srcN[dstBase + c];
        }
        else {
          for(int c = 0; c < cSize; c++)
            dstN[dstBase + c] = srcN[srcBase + c];
        }
      }
    }
  }
  else {
    const int ncStride = area;
    const int ncSize = nSize * cSize;
    for(int nc = 0; nc < ncSize; nc++) {
      const float* srcNC = src + nc * ncStride;
      float* dstNC = dst + nc * ncStride;
      if(reverse) {
        for(int pos = 0; pos < area; pos++)
          dstNC[pos] = srcNC[perm[pos]];
      }
      else {
        for(int pos = 0; pos < area; pos++)
          dstNC[perm[pos]] = srcNC[pos];
      }
    }
  }
}
...
void SymmetryHelpers::copyInputsWithSymmetry(
  const float* src, float* dst, int nSize, int hSize, int wSize, int cSize, bool useNHWC, int symmetry, bool isSnubQuadrangle, int snubGridLen
) {
  if(isSnubQuadrangle)
    copySnubWithSymmetry(src, dst, nSize, hSize, wSize, cSize, useNHWC, symmetry, false, snubGridLen);
  else
    copyWithSymmetry(src, dst, nSize, hSize, wSize, cSize, useNHWC, symmetry, false);
}

void SymmetryHelpers::copyOutputsWithSymmetry(
  const float* src, float* dst, int nSize, int hSize, int wSize, int symmetry, bool isSnubQuadrangle, int snubGridLen
) {
  if(isSnubQuadrangle)
    copySnubWithSymmetry(src, dst, nSize, hSize, wSize, 1, false, symmetry, true, snubGridLen);
  else
...
Loc SymmetryHelpers::getSymLoc(int x, int y, int xSize, int ySize, int symmetry, bool isSnubQuadrangle) {
  if(isSnubQuadrangle && (symmetry & 0x4) == 0) {
    int row = y;
    int col = x;
    SnubQuadrangle::applySymmetryRC(row, col, ySize, xSize, symmetry);
    return Location::getLoc(col, row, xSize);
  }
...
Loc SymmetryHelpers::getSymLoc(int x, int y, const Board& board, int symmetry) {
  return getSymLoc(x,y,board.x_size,board.y_size,symmetry,board.isSnubQuadrangle);
}
...
Loc SymmetryHelpers::getSymLoc(Loc loc, int xSize, int ySize, int symmetry, bool isSnubQuadrangle) {
  if(loc == Board::NULL_LOC || loc == Board::PASS_LOC)
    return loc;
  return getSymLoc(Location::getX(loc,xSize), Location::getY(loc,xSize), xSize, ySize, symmetry, isSnubQuadrangle);
}
...
void SymmetryHelpers::markDuplicateMoveLocs(
  ...
  if(board.isSnubQuadrangle)
    symmetrySearchUpperBound = std::min(symmetrySearchUpperBound, SymmetryHelpers::NUM_SYMMETRIES_WITHOUT_TRANSPOSE);
...
void SymmetryHelpers::getSymmetryDifferences(
  ...
  if(board.isSnubQuadrangle)
    numSymmetries = std::min(numSymmetries, SymmetryHelpers::NUM_SYMMETRIES_WITHOUT_TRANSPOSE);
...
static void iterLadders(const Board& board, int nnXLen, const std::function<void(Loc,int,const vector<Loc>&)>& f) {
  if(Board::MIN_LIBERTIES_TO_SURVIVE > 1)
    return;
  // Snub graph topology breaks ladder search assumptions (e.g. attacker must have exactly 2 liberties).
  // Skipping avoids infinite or near-infinite search during training write (fillRowV*).
  if(board.isSnubQuadrangle)
    return;
  ...
  for(int y = 0; y<ySize; y++) {
    for(int x = 0; x<xSize; x++) {
      if(board.isSnubQuadrangle && !SnubQuadrangle::isValidVertexBoardXY(x, y, xSize, ySize))
        continue;
...
void NNInputs::fillRowV3(
  ...
  for(int y = 0; y<ySize; y++) {
    for(int x = 0; x<xSize; x++) {
      ...
      if(stone == pla || stone == opp) {
        int libs = board.getNumLiberties(loc);
        const int minL = Board::MIN_LIBERTIES_TO_SURVIVE;
        if(libs == minL) setRowBin(rowBin,pos,3, 1.0f, posStride, featureStride);
        else if(libs == minL+1) setRowBin(rowBin,pos,4, 1.0f, posStride, featureStride);
        else if(libs >= minL+2) setRowBin(rowBin,pos,5, 1.0f, posStride, featureStride);
      }
    }
  }
  ...
  auto addLadderFeature = [&board,xSize,nnXLen,nnYLen,posStride,featureStride,rowBin,opp](Loc loc, int pos, const vector<Loc>& workingMoves)  noexcept {
    ...
    if(board.colors[loc] == opp && board.getNumLiberties(loc) > Board::MIN_LIBERTIES_TO_SURVIVE) {
...
void NNInputs::fillRowV7(
  ...
  auto isSnubHole = [&](int x, int y) {
    return board.isSnubQuadrangle && !SnubQuadrangle::isValidVertexBoardXY(x, y, xSize, ySize);
  };
