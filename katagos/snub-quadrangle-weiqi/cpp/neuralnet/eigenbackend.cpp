...
void NeuralNet::getOutput(
  ...
  for(int nIdx = 0; nIdx<batchSize; nIdx++) {
    ...
    SymmetryHelpers::copyInputsWithSymmetry(rowSpatial, rowSpatialInput, 1, nnYLen, nnXLen, numSpatialFeatures, computeHandle->inputsUseNHWC, inputBufs[nIdx]->symmetry, inputBufs[nIdx]->isSnubQuadrangleForServer,
    inputBufs[nIdx]->isSnubQuadrangleForServer ? inputBufs[nIdx]->boardXSizeForServer : 0);
  }
...
#define MAP2(NAME) TENSORMAP2 NAME(buffers.NAME.data(), buffers.NAME.dimension(0), batchSize)
  ...
  for(int row = 0; row < batchSize; row++) {
    ...
    if(numPolicyChannels == 2 || (numPolicyChannels == 4 && modelVersion >= 16)) {
      ...
      SymmetryHelpers::copyOutputsWithSymmetry(policyProbsTmp, policyProbs, 1, nnYLen, nnXLen, inputBufs[row]->symmetry, inputBufs[row]->isSnubQuadrangleForServer,
        inputBufs[row]->isSnubQuadrangleForServer ? inputBufs[row]->boardXSizeForServer : 0);
      policyProbs[nnXLen*nnYLen] = policyPassSrcBuf[0] + (policyPassSrcBuf[1] - policyPassSrcBuf[0]) * policyOptimism;
    }
    else {
      assert(numPolicyChannels == 1);
      SymmetryHelpers::copyOutputsWithSymmetry(policySrcBuf, policyProbs, 1, nnYLen, nnXLen, inputBufs[row]->symmetry, inputBufs[row]->isSnubQuadrangleForServer,
        inputBufs[row]->isSnubQuadrangleForServer ? inputBufs[row]->boardXSizeForServer : 0);
    ...
    if(output->whiteOwnerMap != NULL) {
      ...
      SymmetryHelpers::copyOutputsWithSymmetry(ownershipSrcBuf, output->whiteOwnerMap, 1, nnYLen, nnXLen, inputBufs[row]->symmetry, inputBufs[row]->isSnubQuadrangleForServer,
        inputBufs[row]->isSnubQuadrangleForServer ? inputBufs[row]->boardXSizeForServer : 0);
    }
