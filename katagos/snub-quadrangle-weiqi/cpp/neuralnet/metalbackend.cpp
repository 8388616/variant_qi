...
void MetalProcess::processRowData(size_t row, ComputeHandle* gpuHandle, InputBuffers* inputBuffers, NNResultBuf** inputBufs) {
  ...
  SymmetryHelpers::copyInputsWithSymmetry(
    ...
    inputBufs[row]->isSnubQuadrangleForServer,
...
void MetalProcess::processPolicy(
  ...
  SymmetryHelpers::copyOutputsWithSymmetry(
    targetBuffer, currentOutput->policyProbs, 1, gpuHandle->nnYLen, gpuHandle->nnXLen, inputBuf->symmetry, inputBuf->isSnubQuadrangleForServer,
    inputBuf->isSnubQuadrangleForServer ? inputBuf->boardXSizeForServer : 0);
}
...
void MetalProcess::processOwnership(
  ...
  if(currentOutput->whiteOwnerMap != nullptr) {
    ...
    SymmetryHelpers::copyOutputsWithSymmetry(
      ownershipOutputBuf, currentOutput->whiteOwnerMap, 1, nnYLen, nnXLen, inputBuf->symmetry, inputBuf->isSnubQuadrangleForServer,
      inputBuf->isSnubQuadrangleForServer ? inputBuf->boardXSizeForServer : 0);
  }
}
...
void MetalProcess::processRow(
  ...
  MetalProcess::processOwnership(inputBuffers, currentOutput, gpuHandle, inputBufs[row], row);
