...
// Iterate graph neighbors of LOC; BODY must use variable `adj` (Loc). Snub vs 4-neighbor square.
// Snub branch uses a plain for-loop (not a lambda) so `return` inside BODY returns from the caller, matching
// upstream KataGo behavior (e.g. getNumLibertiesAfterPlay early exit, wouldBeCapture).
#define FOREACHADJ_LOC(LOC, BODY)                                                                                      \
  do {                                                                                                                 \
    if(this->isSnubQuadrangle) {                                                                                       \
      int __sx = Location::getX((LOC), this->x_size);                                                                  \
      int __sy = Location::getY((LOC), this->x_size);                                                                  \
      int __sr[5];                                                                                                   \
      int __sc[5];                                                                                                   \
      int __sn = SnubQuadrangle::collectNeighborRCBoardXY(__sx, __sy, this->x_size, this->y_size, __sr, __sc);                 \
      for(int __si = 0; __si < __sn; __si++) {                                                                       \
        Loc adj = Location::getLoc(__sc[__si], __sr[__si], this->x_size);                                              \
        BODY                                                                                                         \
      }                                                                                                              \
    }                                                                                                                \
    else {                                                                                                           \
      {Loc adj = (LOC) + ADJ0; BODY}                                                                                 \
      {Loc adj = (LOC) + ADJ1; BODY}                                                                                 \
      {Loc adj = (LOC) + ADJ2; BODY}                                                                                 \
      {Loc adj = (LOC) + ADJ3; BODY}                                                                                 \
    }                                                                                                                \
  } while(0)
...
Board::Board(const Board& other)
{
  ...
  isSnubQuadrangle = other.isSnubQuadrangle;
}
...
void Board::init(int xS, int yS)
{
  ...
  isSnubQuadrangle = false;
}

void Board::initSnubQuadrangleBoard(int lanes) {
  if(lanes < SnubQuadrangle::MIN_LANES || lanes > SnubQuadrangle::MAX_LANES)
    throw StringError("Board::initSnubQuadrangleBoard: lanes must be in [2,8]");
  int g = SnubQuadrangle::gridLenFromLanes(lanes);
  init(g,g);
  isSnubQuadrangle = true;
  for(int y = 0; y < y_size; y++) {
    for(int x = 0; x < x_size; x++) {
      if(!SnubQuadrangle::isValidVertexBoardXY(x,y,x_size,y_size)) {
  ...
  SnubQuadrangle::warmSymmetryCache(g);
}

void Board::ensureSnubQuadrangleTopology() {
  if(isSnubQuadrangle)
    return;
  if(x_size != y_size || !SnubQuadrangle::isSnubGridLen(x_size))
    return;
  isSnubQuadrangle = true;
  for(int y = 0; y < y_size; y++) {
    for(int x = 0; x < x_size; x++) {
  ...
  SnubQuadrangle::warmSymmetryCache(x_size);
}
...
bool Board::isSuicide(Loc loc, Player pla) const
{
  ...
  if(pla != P_BLACK && pla != P_WHITE)
    return false;
  return getNumLibertiesAfterPlay(loc, pla, MIN_LIBERTIES_TO_SURVIVE) < MIN_LIBERTIES_TO_SURVIVE;
}
...
void Board::getBoundNumLibertiesAfterPlay(Loc loc, Player pla, int& lowerBound, int& upperBound) const
{
  ...
  FOREACHADJ_LOC(loc,
    if(colors[adj] == C_EMPTY) {
      ...
      if(libs <= MIN_LIBERTIES_TO_SURVIVE) {
        numCaps++;
...
int Board::getNumLibertiesAfterPlay(Loc loc, Player pla, int max) const
{
  ...
  FOREACHADJ_LOC(loc,
    ...
    else if(colors[adj] == opp && getNumLiberties(adj) <= MIN_LIBERTIES_TO_SURVIVE) {
      libs[numLibs++] = adj;
      if(numLibs >= max)
        return max;
...
bool Board::isLegalIgnoringKo(Loc loc, Player pla, bool isMultiStoneSuicideLegal) const
{
  ...
  return loc == PASS_LOC || (
    ...
    loc < MAX_ARR_SIZE &&
    (colors[loc] == C_EMPTY) &&
    (!isSnubQuadrangle || SnubQuadrangle::isValidVertexBoardXY(Location::getX(loc,x_size), Location::getY(loc,x_size), x_size, y_size)) &&
    !isIllegalSuicide(loc, pla, isMultiStoneSuicideLegal)
  );
}
...
bool Board::wouldBeCapture(Loc loc, Player pla) const {
  ...
  FOREACHADJ_LOC(loc,
    if(colors[adj] == opp)
    {
      if(getNumLiberties(adj) <= MIN_LIBERTIES_TO_SURVIVE)
        return true;
    }
  );
...
bool Board::isNonPassAliveSelfConnection(Loc loc, Player pla, const Color* passAliveArea) const {
  ...
  FOREACHADJ_LOC(loc,
    if(nonPassAliveAdjHead == NULL_LOC && colors[adj] == pla && passAliveArea[adj] == C_EMPTY)
...
void Board::rebuildAllChainsFromColors() {
  for(int y = 0; y < y_size; y++) {
    for(int x = 0; x < x_size; x++) {
      Loc loc = Location::getLoc(x,y,x_size);
      if(colors[loc] == P_BLACK || colors[loc] == P_WHITE)
        chain_head[loc] = NULL_LOC;
    }
  }
  for(int y = 0; y < y_size; y++) {
    for(int x = 0; x < x_size; x++) {
      ...
      if((pla == P_BLACK || pla == P_WHITE) && chain_head[loc] == NULL_LOC)
        rebuildChain(loc, pla);
    }
  }
}
...
bool Board::setStonesFailIfNoLibs(const std::vector<Move>& placements) {
  ...
  for(size_t i = 0; i < placements.size(); i++) {
    Loc loc = placements[i].loc;
    if(locs.find(loc) != locs.end())
      return false;
    if(isSnubQuadrangle) {
  ...
  for(size_t i = 0; i < placements.size(); i++) {
    Loc loc = placements[i].loc;
    Player pla = placements[i].pla;
    if(colors[loc] != C_EMPTY)
      return false;
    colors[loc] = pla;
    pos_hash ^= ZOBRIST_BOARD_HASH[loc][pla];
  }
  rebuildAllChainsFromColors();
  ko_loc = NULL_LOC;
  for(size_t i = 0; i < placements.size(); i++) {
    Loc loc = placements[i].loc;
    Player pla = placements[i].pla;
    if(colors[loc] != pla)
      return false;
    if(getNumLiberties(loc) < MIN_LIBERTIES_TO_SURVIVE)
      return false;
  }
  return true;
}
...
Board::MoveRecord Board::playMoveRecorded(Loc loc, Player pla)
{
  ...
  if(loc != PASS_LOC) {
...

    {
      int capBit = 0;
      FOREACHADJ_LOC(loc,
        if(colors[adj] == opp && getNumLiberties(adj) <= MIN_LIBERTIES_TO_SURVIVE)
          record.capDirs |= (uint8_t)(((uint8_t)1) << capBit);
        capBit++;
      );
    }
...
void Board::undo(Board::MoveRecord record)
{
  ...
  //Re-fill stones in all captured directions (bit index matches playMoveRecorded / neighbor iteration order)
  {
    int capBit = 0;
    FOREACHADJ_LOC(loc,
      if(record.capDirs & (1 << capBit)) {
        if(colors[adj] == C_EMPTY) {
...
Hash128 Board::getPosHashAfterMove(Loc loc, Player pla) const {
  ...
  // Multi-liberty rules: incremental hash is not implemented; fall back to simulating the move.
  // Snub quadrangle: incremental path does not model self-capture after captures; simulate for superko.
  if(MIN_LIBERTIES_TO_SURVIVE > 1 || isSnubQuadrangle) {
    Board copy = *this;
  ...
  if(isSnubQuadrangle) {
    int sr[5];
    int sc[5];
    int sn = SnubQuadrangle::collectNeighborRCBoardXY(
      Location::getX(loc, x_size), Location::getY(loc, x_size), x_size, y_size, sr, sc);
    for(int i = 0; i < sn; i++) {
      Loc adj = Location::getLoc(sc[i], sr[i], x_size);
      if(colors[adj] == C_EMPTY)
        wouldBeSuicide = false;
      else if(colors[adj] == pla && getNumLiberties(adj) > MIN_LIBERTIES_TO_SURVIVE)
        wouldBeSuicide = false;
      else if(colors[adj] == opp) {
        if(getNumLiberties(adj) <= MIN_LIBERTIES_TO_SURVIVE) {
          Loc head = chain_head[adj];
    ...
    if(wouldBeSuicide) {
      ...
      for(int i = 0; i < sn; i++) {
        Loc adj = Location::getLoc(sr[i], sc[i], x_size);
        if(colors[adj] == pla && getNumLiberties(adj) <= MIN_LIBERTIES_TO_SURVIVE) {
          Loc head = chain_head[adj];
          bool alreadyFound = false;
          for(int j = 0; j < numCapturedGroups; j++) {
            if(capturedGroupHeads[j] == head) {
...
void Board::playMoveAssumeLegal(Loc loc, Player pla)
{
  ...
  Loc opp_heads_seen[8];   //Heads of the opp chains seen so far

  // Use per-neighbor closure so "skip neighbor" does not return from this function (FOREACHADJ non-snub uses bare
  // blocks; snub uses a for-loop — both interpret `return` as exiting playMoveAssumeLegal).
  auto handleNeighbor = [&](Loc adj) {
    if(colors[adj] == pla) {
  ...
  if(isSnubQuadrangle) {
    ...
    for(int si = 0; si < sn; si++)
      handleNeighbor(Location::getLoc(sc[si], sr[si], x_size));
  }
  else {
    for(int i = 0; i < 4; i++)
      handleNeighbor(loc + adj_offsets[i]);
  }
  ...
  if(MIN_LIBERTIES_TO_SURVIVE <= 1 &&
     num_captured == 1 && chain_data[chain_head[loc]].num_locs == 1 && chain_data[chain_head[loc]].num_liberties == 1)
...
int Board::getNumImmediateLiberties(Loc loc) const
{
  int num_libs = 0;
  FOREACHADJ_LOC(loc,
    if(colors[adj] == C_EMPTY) num_libs++;
  );
  return num_libs;
}
...
Loc Board::rebuildChainHelper(Loc head, Loc tailTarget, Loc loc, Player pla)
{
  ...
  FOREACHADJ_LOC(loc,
    if(colors[adj] == C_EMPTY && !isLibertyOf(adj,head)) numHeadLibertiesToAdd++;
  );
  ...
  Loc nextTailTarget = loc;
  FOREACHADJ_LOC(loc,
    if(colors[adj] == pla && chain_head[adj] != head) nextTailTarget = rebuildChainHelper(head,nextTailTarget,adj,pla);
  );
  return nextTailTarget;
}
...
void Board::changeSurroundingLiberties(Loc loc, Player pla, int delta)
{
  Loc headsSeen[12];
  int nh = 0;
  auto applyToPlaHead = [&](Loc adj) {
    if(colors[adj] != pla)
      return;
    Loc h = chain_head[adj];
    for(int i = 0; i < nh; i++)
      if(headsSeen[i] == h)
        return;
    headsSeen[nh++] = h;
    chain_data[h].num_liberties += delta;
  };
  if(isSnubQuadrangle) {
    int sr[5];
    int sc[5];
    int sn = SnubQuadrangle::collectNeighborRCBoardXY(
      Location::getX(loc, x_size), Location::getY(loc, x_size), x_size, y_size, sr, sc);
    for(int si = 0; si < sn; si++)
      applyToPlaHead(Location::getLoc(sc[si], sr[si], x_size));
  }
  else {
    for(int i = 0; i < 4; i++)
      applyToPlaHead(loc + adj_offsets[i]);
  }
}
...
int Board::findLibertyGainingCaptures(Loc loc, vector<Loc>& buf, int bufStart, int bufIdx) const {
  ...
  do
  {
    FOREACHADJ_LOC(cur,
      if(colors[adj] == opp) {
        Loc head = chain_head[adj];
        if(chain_data[head].num_liberties <= MIN_LIBERTIES_TO_SURVIVE) {
...
void Board::calculateArea(
  ...
  if(nonPassAliveStones) {
    for(int y = 0; y < y_size; y++) {
      for(int x = 0; x < x_size; x++) {
        if(isSnubQuadrangle && !SnubQuadrangle::isValidVertexBoardXY(x, y, x_size, y_size))
          continue;
        Loc loc = Location::getLoc(x,y,x_size);
...
void Board::calculateAreaForPla(
  ...
  for(int y = 0; y < y_size; y++) {
    for(int x = 0; x < x_size; x++) {
      ...
      {
        uint16_t vStart = vitalStart[regionIdx];
        assert(vStart + 8 <= vitalForPlaHeadsListsMaxLen);
    ...
    for(int x = 0; x < x_size; x++) {
      ...
      if(basicArea[loc] != C_EMPTY && !isSeki[loc]) {
        if(
          ...
          (colors[loc] == basicArea[loc] && getNumLiberties(loc) < MIN_LIBERTIES_TO_SURVIVE) ||
          //Touches dame? Treat as seki (graph neighbors on snub board)
          ([&]() {
            bool td = false;
            FOREACHADJ_LOC(loc,
              if(colors[adj] == C_EMPTY && basicArea[adj] == C_EMPTY)
                td = true;
            );
            return td;
          })()
...
void Board::checkConsistency() const {
  ...
  auto checkChainConsistency = [&buf,errLabel,&chainLocChecked,this](Loc loc) {
    ...
    if(data.num_liberties < MIN_LIBERTIES_TO_SURVIVE)
      throw StringError(errLabel + "Chain has fewer liberties than required to survive");
  ...
  for(Loc loc = 0; loc < MAX_ARR_SIZE; loc++) {
    ...
    if(x < 0 || x >= x_size || y < 0 || y >= y_size) {
      ...
      // Snub quadrangle: embedding has non-playable cells inside the x_size*y_size rect; they stay C_WALL.
      if(isSnubQuadrangle && !SnubQuadrangle::isValidVertexBoardXY(x, y, x_size, y_size)) {
        if(colors[loc] != C_WALL)
          throw StringError(errLabel + "Snub non-vertex must be WALL");
        continue;
      }
      if(colors[loc] == C_BLACK || colors[loc] == C_WHITE) {
