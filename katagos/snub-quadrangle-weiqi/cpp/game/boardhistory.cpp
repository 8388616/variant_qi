...
bool BoardHistory::isLegal(const Board& board, Loc moveLoc, Player movePla) const {
  ...
  if(encorePhase > 0) {
    ...
    //扭棱四角围棋：禁全同由 positional superko 处理，不使用单点劫位
    if(Board::MIN_LIBERTIES_TO_SURVIVE <= 1 && board.isKoBanned(moveLoc))
      return false;
  }
  if(!board.isLegalIgnoringKo(moveLoc,movePla,rules.multiStoneSuicideLegal))
    return false;
  // Pass is exempt from positional superko ban (PASS_LOC is not marked in the superKoBanned loop).
  if(moveLoc != Board::PASS_LOC && superKoBanned[moveLoc])
    return false;
...
void BoardHistory::makeBoardMoveAssumeLegal(Board& board, Loc moveLoc, Player movePla, const KoHashTable* rootKoHashTable, bool preventEncore) {
  ...
  // Positional superko: pass does not change stones; do not duplicate board hash in history.
  bool recordKoHash = true;
  if(moveLoc == Board::PASS_LOC && rules.koRule == Rules::KO_POSITIONAL && koHashHistory.size() > 0) {
    if(koHashAfterThisMove == koHashHistory[koHashHistory.size()-1])
      recordKoHash = false;
  }
  if(recordKoHash)
  ...
  if(encorePhase <= 0 && rules.koRule != Rules::KO_SIMPLE) {
    ...
    for(int y = 0; y<board.y_size; y++) {
      for(int x = 0; x<board.x_size; x++) {
        ...
        // If the move returns to the same board (e.g. single-stone self-capture), must still check superko even when never played.
        // (Does not apply to PASS_LOC: pass is not in this loop; legality/history for pass are handled separately below.)
        else if(Board::MIN_LIBERTIES_TO_SURVIVE <= 1 && !wasEverOccupiedOrPlayed[loc] && !board.isSuicide(loc,nextPla)) {
          Hash128 posHashAfterMove = board.getPosHashAfterMove(loc,nextPla);
          if(posHashAfterMove != board.pos_hash)
            superKoBanned[loc] = false;
          else {
        ...
        else if(board.isIllegalSuicide(loc,nextPla,rules.multiStoneSuicideLegal) ||
                (Board::MIN_LIBERTIES_TO_SURVIVE <= 1 && loc == board.ko_loc))
          superKoBanned[loc] = false;
        else {
          Hash128 posHashAfterMove = board.getPosHashAfterMove(loc,nextPla);
          Hash128 koHashAfterMove = getKoHashAfterMoveNonEncore(rules, posHashAfterMove, getOpp(nextPla));
