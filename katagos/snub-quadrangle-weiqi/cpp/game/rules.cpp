...
Rules::Rules() {
  //扭棱四角围棋：全同形禁着（positional superko）；黑贴白 3.25 子 = KataGo komi 6.5 目
...
bool Rules::komiIsIntOrHalfInt(float komi) {
  // Variant qi: accept arbitrary finite komi (e.g. 2.8 points / 1.4 zi), not only half-integers.
  return std::isfinite(komi);
}
    ...
    try {
      ...
      for(json::iterator iter = input.begin(); iter != input.end(); ++iter) {
        ...
        if(key == "ko")
          ...
          komiSpecified = true;
          if(rules.komi < Rules::MIN_USER_KOMI || rules.komi > Rules::MAX_USER_KOMI)
            throw IOError("Komi value is too extreme");
        }
        else
          throw IOError("Unknown rules option: " + key);
      }
