...
// Snub-quadrangle weiqi: standard capture (no liberties => capture); superko in BoardHistory.
...
struct Board
{
  ...
  static constexpr int DEFAULT_LEN = std::min(MAX_LEN,9); //Default edge length for board if unspecified

  // Same as standard Go: chains need at least one liberty to avoid capture.
  static constexpr int MIN_LIBERTIES_TO_SURVIVE = 1;
  ...
  // Snub-quadrangle board: lanes in [2,8], grid edge 3*lanes-2 (variant_qi layout).
  void initSnubQuadrangleBoard(int lanes);
  // Mark holes / set flag on an existing position (e.g. SGF startpos) without clearing stones.
  void ensureSnubQuadrangleTopology();
  ...
  //Sets multiple stones: write all colors, rebuildAllChainsFromColors(), then validate final liberties.
  //Uses snub graph adjacency in rebuildChain. Returns false on duplicate loc, invalid vertex, or
  //any placed group with fewer than MIN_LIBERTIES_TO_SURVIVE liberties.
  ...
  // If true: graph adjacency from snub-square tiling on a square grid; invalid cells are C_WALL.
  bool isSnubQuadrangle;
  ...
  void rebuildAllChainsFromColors();
