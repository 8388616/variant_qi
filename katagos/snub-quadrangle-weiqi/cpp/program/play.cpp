...
void GameInitializer::initShared(ConfigParser& cfg, Logger& logger) {
  ...
  snubQuadrangleBoard = cfg.contains("snubQuadrangleBoard") && cfg.getBool("snubQuadrangleBoard");
  if(snubQuadrangleBoard && cfg.contains("bSizesXY"))
    throw IOError("snubQuadrangleBoard requires bSizes (lane list), not bSizesXY, in " + cfg.getFileName());
  ...
  if(cfg.contains("bSizes")) {
    ...
    if(snubQuadrangleBoard) {
      std::vector<int> allowedLanes = cfg.getInts("bSizes", SnubQuadrangle::MIN_LANES, SnubQuadrangle::MAX_LANES);
      if(allowedLanes.size() <= 0)
        throw IOError("bSizes must have at least one value in " + cfg.getFileName());
      if(allowedLanes.size() != allowedBEdgeRelProbs.size())
      ...
      if(cfg.contains("allowRectangleProb") && cfg.getDouble("allowRectangleProb",0.0,1.0) > 0.0)
        throw IOError("snubQuadrangleBoard does not support allowRectangleProb in " + cfg.getFileName());

      allowedBSizes.clear();
      allowedBSizeRelProbs.clear();
      for(int i = 0; i < (int)allowedLanes.size(); i++) {
        int lane = allowedLanes[i];
        int g = SnubQuadrangle::gridLenFromLanes(lane);
        if(g > Board::MAX_LEN)
          throw IOError("Snub lane count implies grid " + Global::intToString(g) + " but COMPILE_MAX_BOARD_LEN is " + Global::intToString(Board::MAX_LEN) + " in " + cfg.getFileName());
        if(!SnubQuadrangle::isSnubGridLen(g))
          throw IOError("Invalid snub lane / grid in bSizes in " + cfg.getFileName());
        allowedBSizes.emplace_back(g, g);
        allowedBSizeRelProbs.push_back(allowedBEdgeRelProbs[i] / relProbSum);
      }
    }
    else {
  ...
  komiMean = cfg.contains("komiMean") ? cfg.getFloat("komiMean",Rules::MIN_USER_KOMI,Rules::MAX_USER_KOMI) : 3.25f;
...
void GameInitializer::createGameSharedUnsynchronized(
  ...
  if(initialPosition != NULL) {
    ...
    if(snubQuadrangleBoard)
      board.ensureSnubQuadrangleTopology();
  ...
  if(posSample != NULL) {
    ...
    if(snubQuadrangleBoard) {
      testAssert(xSize == ySize);
      board.initSnubQuadrangleBoard(SnubQuadrangle::lanesFromGridLen(xSize));
    }
    else
...
FinishedGameData* Play::runGame(
  ...
  bool recordFullData = playSettings.forSelfPlay || playSettings.recordTrainingData;
...
FinishedGameData* GameRunner::runGame(
  ...
  if(playSettings.forSelfPlay) {
    ...
    if(botSpecB.botIdx == botSpecW.botIdx) {
      ...
      // Match training (two nets): share board/komi randomization only, keep per-bot search params.
