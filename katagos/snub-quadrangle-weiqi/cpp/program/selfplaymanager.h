...
class SelfplayManager {
 public:
  ...
  //After all game threads have finished enqueueing: mark write queues read-only so the data
  //write loop can drain and exit (instead of blocking on an empty queue until destructor).
  void markGameQueuesReadOnly();

  //Block until all data-write threads finish (queues must already be read-only or empty).
  void waitForDataWriteLoopsDone();
