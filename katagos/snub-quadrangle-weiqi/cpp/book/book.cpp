...
void BookHash::getHashAndSymmetry(const BoardHistory& hist, int repBound, BookHash& hashRet, int& symmetryToAlignRet, vector<int>& symmetriesRet, int bookVersion) {
  ...
  // Snub quadrangle boards only use flip symmetries (same as NN / search).
  ...
  if(hist.getRecentBoard(0).isSnubQuadrangle)
...
BookMove BookMove::getSymBookMove(int symmetry, const Board& board) const {
  BookMove ret(
    SymmetryHelpers::getSymLoc(move,board,symmetry),
...
vector<BookMove> ConstSymBookNode::getUniqueMovesInBook() const {
  ...
  for(std::pair<Loc,BookMove> kv: node->moves) {
    ret.push_back(kv.second.getSymBookMove(symmetryOfNode, node->book->initialBoard));
  }
  return ret;
}
...
SymBookNode SymBookNode::playAndAddMove(Board& board, BoardHistory& hist, Loc move, double rawPolicy, bool& childIsTransposing) {
  ...
  Loc symMove = SymmetryHelpers::getSymLoc(move, node->book->initialBoard, invSymmetryOfNode);
  ...
  for(int symmetry: node->symmetries) {
    if(symmetry == 0)
      continue;
    Loc symLoc = SymmetryHelpers::getSymLoc(symMove, node->book->initialBoard, symmetry);
...
Book::MinCostResult Book::computeMinCostToChangeWinLossHelper(
  ...
  if((node->pla == P_WHITE && increasing) || (node->pla == P_BLACK && !increasing)) {
    ...
    for(auto iter = node->moves.begin(); iter != node->moves.end(); ++iter) {
      const BookNode* child = get(iter->second.hash);
      MinCostResult childResult = computeMinCostToChangeWinLossHelper(child, costFunc, edgeCost, winLossValueThreshold, increasing, pruneOverCost, costCache, resultCache, visited);
      childResult.totalCost += edgeCost(ConstSymBookNode(node,0), iter->second.getSymBookMove(0, node->book->initialBoard));
