...
PlaySettings PlaySettings::loadForMatch(ConfigParser& cfg) {
  ...
  // Optional: same visit-throttling knobs as selfplay (so match can load overrides_fast.cfg without those keys being unused).
  if(cfg.contains("estimateLeadVisits"))
    playSettings.estimateLeadVisits = cfg.getInt("estimateLeadVisits",1,10000);
  if(cfg.contains("estimateLeadProb"))
    playSettings.estimateLeadProb = cfg.getDouble("estimateLeadProb",0.0,1.0);

  if(cfg.contains("cheapSearchProb"))
    playSettings.cheapSearchProb = cfg.getDouble("cheapSearchProb",0.0,1.0);
  if(cfg.contains("cheapSearchVisits"))
    playSettings.cheapSearchVisits = cfg.getInt("cheapSearchVisits",1,10000000);
  if(cfg.contains("cheapSearchTargetWeight"))
    playSettings.cheapSearchTargetWeight = cfg.getFloat("cheapSearchTargetWeight",0.0f,1.0f);

  if(cfg.contains("reduceVisits"))
    playSettings.reduceVisits = cfg.getBool("reduceVisits");
  if(cfg.contains("reduceVisitsThreshold"))
    playSettings.reduceVisitsThreshold = cfg.getDouble("reduceVisitsThreshold",0.0,0.999999);
  if(cfg.contains("reduceVisitsThresholdLookback"))
    playSettings.reduceVisitsThresholdLookback = cfg.getInt("reduceVisitsThresholdLookback",0,1000);
  if(cfg.contains("reducedVisitsMin"))
    playSettings.reducedVisitsMin = cfg.getInt("reducedVisitsMin",1,10000000);
  if(cfg.contains("reducedVisitsWeight"))
    playSettings.reducedVisitsWeight = cfg.getFloat("reducedVisitsWeight",0.0f,1.0f);

  if(playSettings.cheapSearchProb > 0.0 && playSettings.cheapSearchVisits <= 0)
    throw StringError("cheapSearchProb > 0 requires cheapSearchVisits > 0 in config");

  playSettings.recordTimePerMove = true;
  playSettings.recordTrainingData = cfg.contains("writeTdata") && cfg.getBool("writeTdata");
  return playSettings;
}
