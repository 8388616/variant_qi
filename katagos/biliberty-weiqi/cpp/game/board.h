...
//二气围棋：每块棋至少 MIN_LIBERTIES_TO_SURVIVE 口气；劫形由 BoardHistory 的禁全同（positional superko）处理。
//Does not enforce player turn order.
static constexpr int MIN_LIBERTIES_TO_SURVIVE = 2;
...
struct Board
{
  ...
  //Allows final legal positions whose intermediate sequential placement would fail. Returns false on
  //duplicate loc or any placed group with fewer than MIN_LIBERTIES_TO_SURVIVE liberties.
