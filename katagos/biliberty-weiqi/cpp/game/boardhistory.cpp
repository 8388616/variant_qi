...
bool BoardHistory::isLegal(const Board& board, Loc moveLoc, Player movePla) const {
  ...
  if(encorePhase > 0) {
    ...
    //二气围棋：禁全同由 positional superko 处理，不使用单点劫位
    if(MIN_LIBERTIES_TO_SURVIVE <= 1 && board.isKoBanned(moveLoc))
      return false;
  }
  ...
  if(encorePhase <= 0 && rules.koRule != Rules::KO_SIMPLE) {
    ...
    for(int y = 0; y<board.y_size; y++) {
      for(int x = 0; x<board.x_size; x++) {
        ...
        // Biliberty: self-capture may leave loc empty; do not skip superko for never-played points.
        else if(MIN_LIBERTIES_TO_SURVIVE <= 1 && !wasEverOccupiedOrPlayed[loc] && !board.isSuicide(loc,nextPla))
          superKoBanned[loc] = false;
        else if(board.isIllegalSuicide(loc,nextPla,rules.multiStoneSuicideLegal) ||
                (MIN_LIBERTIES_TO_SURVIVE <= 1 && loc == board.ko_loc))
          superKoBanned[loc] = false;
        else {
