...
return {
    ...
    pieceLabel
        ...
        const SLOT_UI = {
            player1: { name: '蓝方', emoji: '🔵', continueText: '继续执蓝', choiceText: '执蓝', youText: '您执蓝', absentText: '蓝方已退出', statusText: '蓝方' },
            player2: { name: '红方', emoji: '🔴', continueText: '继续执红', choiceText: '执红', youText: '您执红', absentText: '红方已退出', statusText: '红方' }
        };
        ...
        function toDisplayCoord(row, col) {
            // 蓝方在 row 小的一侧且显示在下：先做视角 180° 旋转，再整体 y 镜像
            let r = row, c = col;
            ...
            for (let r = 0; r < R.BOARD_H; r++) {
                for (let c = 0; c < R.BOARD_W; c++) {
                    ...
                    const color = piece[0] === 'r' ? '#1a5fa8' : '#932c13';
                                                                                // 朝鲜将棋式：正面与侧壁阴影同为「正八角形」且顶部对齐——外圈更大、顶边齐平，
                    // 形似八角棱柱侧壁的投影。EXTRUDE 越大侧壁越宽。
                    const EXTRUDE = radius * 0.12;
                    const apothem = (r) => r * Math.cos(Math.PI / 8);
                    const outerRad = radius + EXTRUDE;
                    // 顶边对齐：外圈圆心比正面圆心低（外圈边心距 - 正面边心距）
                    const cyOuter = y + (apothem(outerRad) - apothem(radius));
                    ctx2d.shadowOffsetY = radius * 0.12;
                    ctx2d.shadowBlur = radius * 0.22;
                    ctx2d.shadowColor = 'rgba(0,0,0,0.35)';
                    pathOctagon(x, cyOuter, outerRad);
                    ctx2d.fillStyle = 'rgba(200,160,104,1)';
                    ctx2d.fill();
                    ctx2d.shadowBlur = 0; ctx2d.shadowOffsetY = 0;
                    pathOctagon(x, y, radius);
                    ctx2d.fillStyle = '#e8d2a0';
                    ctx2d.fill();
                    ctx2d.strokeStyle = 'rgba(200,160,104,1)';
                    ctx2d.lineWidth = 1.5;
                    pathOctagon(x, cyOuter, outerRad);
                    ctx2d.stroke();
                    ctx2d.fillStyle = color;
                    ctx2d.font = `${cellSize * 0.60 * sizeScale}px XiangqiPiece`;
                    ctx2d.textAlign = 'center';
                    ctx2d.textBaseline = 'middle';
        ...
        function updateTurn() {
            if (ps.gameOver) {
                ...
                else if (ps.winner === 'player1') text = '🔵 蓝方胜';
                else if (ps.winner === 'player2') text = '🔴 红方胜';
            ...
            if (ps.openingSetup && !ps.tryPlayMode && !ps.replayMode) {
                turnDisplay.innerText = ps.openingSetup === 'player1'
                    ? '🔴 红方配置开局'
                    : '🔵 蓝方配置开局';
                scoreTitle.innerText = '　';
                leadInfo.innerText = '　';
            ...
            if (!ps.tryPlayMode && !ps.replayMode) {
                ...
                const moverLabel = side === 'red' ? '红方' : '蓝方';
        ...
        function enterReplayMode(data) {
            ...
            for (const raw of moves) {
                ...
                if (typeof raw === 'string') {
                    const mp = raw.match(/^([LR])p$/i);
                    if (mp) {
                        side = R.oppositeSide(side);
                        snaps.push(snapshotFromBoard(b, side, null, null));
                        continue;
                    }
                    const mt = raw.match(/^([LR])(\d+),(\d+)-(\d+),(\d+)$/i);
                    if (!mt) continue;
