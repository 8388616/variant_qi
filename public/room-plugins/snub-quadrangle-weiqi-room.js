...
const scoreTitle = document.getElementById('scoreTitle');
        ...
        function bensonInfoSnub(bd) {
            return window.QiWeiqiSquarePageRuntime.bensonAliveGrid({
                width: GRID_H, height: GRID_W,
                getNeighbors: (r, c) => Snub.getNeighbors(r, c, GRID_W, GRID_H),
                isValid: (r, c) => Snub.isValidVertex(r, c, GRID_W, GRID_H),
                get: (r, c) => bd[r][c]
            });
        }
        function bensonSnubRemoveDead(srcBoard) {
            const benson = bensonInfoSnub(srcBoard);
        ...
        function bensonSnubTerritory(liveBoard) {
            const territory = assignTerritoryWithRange(liveBoard);
            const secure = bensonInfoSnub(liveBoard);
            for (let r = 0; r < GRID_W; r++) {
                for (let c = 0; c < GRID_H; c++) {
        ...
        function computeScoreFromBoard(srcBoard) {
            const liveBoard = bensonSnubRemoveDead(srcBoard);
            const territory = bensonSnubTerritory(liveBoard);
        ...
        function showEstimate()
        {
            ...
            const r = QiSquareWeiqiCanvas.computeWeiqiEstimateCaches(
                board, bensonSnubRemoveDead, bensonSnubTerritory, computeScore, KOMI
            );
...
syncState,
        ...
        if (isMouseDevice)
        {
            canvas.addEventListener('mousemove', (e) => {
                ...
                isHoverValid = (row >= 0 && col >= 0 && Snub.isValidVertex(row, col, GRID_W, GRID_H)
