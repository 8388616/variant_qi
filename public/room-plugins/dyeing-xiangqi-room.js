...
return {
    ...
    ownerColorHex
        ...
        const SLOT_UI = {
            ...
            player2: { name: '绿方', emoji: '🟢', continueText: '继续执绿', choiceText: '执绿', youText: '您执绿', absentText: '绿方已退出', statusText: '绿方' }
        };
...
const canvas = document.getElementById('goBoard');
        ...
        function updateTurn() {
            if (ps.gameOver) {
                ...
                else if (ps.winner === 'player2') text = '🟢 绿方胜';
            ...
            if (!ps.tryPlayMode && !ps.replayMode) {
                ...
                const moverLabel = side === 'red' ? '绿方' : '红方';
        ...
        function enterReplayMode(data) {
            ...
            for (const raw of moves) {
                ...
                if (typeof raw === 'string') {
                    const mt = raw.match(/^([RG])(\d+),(\d+)-(\d+),(\d+)$/i);
                    if (!mt) continue;
