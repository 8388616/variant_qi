...
window.RoomPlugins["radar-weiqi"] = {
    ...
    mount: function (ctx) {
        ...
        var recordDownloadPrefix = config.recordDownloadPrefix != null ? config.recordDownloadPrefix : "雷达围棋";
...
const ps = {
            ...
            /** 服务端下发的曼哈顿距离；联网局面浏览用 liveReplayOpponentDistanceGrids；导入棋谱打谱用 replayTruthBoards 本地计算 */
            serverOpponentDistanceGrid: null,
            liveReplayOpponentDistanceGrids: []
        };
...
const scoreTitle = document.getElementById('scoreTitle');
        ...
        /** 基于完整局面：到最近一枚对方棋子的曼哈顿距离；无对方棋子时为 null。 */
        function nearestOpponentManhattanFromTruth(truthBoard, size, row, col) {
            const my = truthBoard[row][col];
            if (my !== 1 && my !== 2) return null;
            const opp = 3 - my;
            let best = Infinity;
            for (let r = 0; r < size; r++) {
                for (let c = 0; c < size; c++) {
                    if (truthBoard[r][c] !== opp) continue;
        ...
        function cloneOpponentDistanceGrid(g) {
            if (!g || !g.length) return null;
            return g.map(row => (row ? row.slice() : row));
        }

        function cloneOpponentDistanceGrids(arr) {
            if (!arr || !arr.length) return [];
            return arr.map(cloneOpponentDistanceGrid);
        }

        /**
         * 联网局面浏览：仅当「完整局面」与「本座可见局面」在任一格不同时，才可用 truth 回退算曼哈顿距离；
         * 二者始终相同则说明未持有完整局面，避免误算。
         */
        function liveReplayTruthBoardForDistanceIfFull(step) {
            const tb = ps.liveReplayTruthBoards && ps.liveReplayTruthBoards[step];
            const vb = ps.liveReplayBoards && ps.liveReplayBoards[step];
            if (!tb || !vb) return null;
            for (let r = 0; r < ps.BOARD_SIZE; r++) {
                for (let c = 0; c < ps.BOARD_SIZE; c++) {
                    if (tb[r][c] !== vb[r][c]) return tb;
                }
            }
            return null;
        }
        ...
        /**
         * 当前格到最近对方子的曼哈顿距离（与对弈时服务端网格语义一致：对方含盘上全部对方子，含隐身子）。
         * 打谱：用本地完整局面计算；联网：优先服务端下发的距离网格。
         */
        function resolveOpponentManhattanDistance(row, col) {
            const size = ps.BOARD_SIZE;
            if (ps.replayMode && !ps.tryPlayMode) {
                const n = ps.replayTruthBoards.length;
                const truth = n > 0
                    ? ps.replayTruthBoards[Math.min(ps.replayStep, n - 1)]
                    : (ps.replayBoards.length > 0
                        ? ps.replayBoards[Math.min(ps.replayStep, ps.replayBoards.length - 1)]
                        : null);
                if (!truth) return null;
                return nearestOpponentManhattanFromTruth(truth, size, row, col);
            }
            if (ps.replayMode && ps.tryPlayMode)
                return nearestOpponentManhattanFromTruth(ps.board, size, row, col);
            if (ps.liveReplayOpponentDistanceGrids && ps.liveReplayOpponentDistanceGrids.length > 0
                && ps.liveViewStep >= 0 && ps.liveViewStep < ps.liveReplayOpponentDistanceGrids.length) {
                const g = ps.liveReplayOpponentDistanceGrids[ps.liveViewStep];
                if (g && g[row] && g[row][col] != null)
                    return g[row][col];
            }
            if (ps.serverOpponentDistanceGrid && ps.serverOpponentDistanceGrid[row]
                && ps.serverOpponentDistanceGrid[row][col] != null)
                return ps.serverOpponentDistanceGrid[row][col];
            const lt = liveReplayTruthBoardForDistanceIfFull(ps.liveViewStep);
            if (lt)
                return nearestOpponentManhattanFromTruth(lt, size, row, col);
            return null;
        }
        ...
        function getStoneColorForInvisibleTintAt(rr, cc) {
            ...
            if (ps.mySlot === 'player1') return 1;
            if (ps.mySlot === 'player2') return 2;
            return 0;
        }

        function drawOpponentDistanceLabels(ctx) {
            if (ps.showMoveNumbers || ps.showEstimateActive) return;
            const size = ps.BOARD_SIZE;
            const displayBoard = ps.board;
            const cellSize = ps.CELL_SIZE;
            const padding = ps.PADDING;
            const stoneRadius = cellSize * 0.44;
            for (let row = 0; row < size; row++) {
                for (let col = 0; col < size; col++) {
                    if (displayBoard[row][col] === 0) continue;
                    ...
                    const textX = cx - stoneRadius * 0.4;
                    const textY = cy - stoneRadius * 0.4;
                    let textColor;
                    if (val === 1)
                        textColor = '#ffffff';
                    else
                    ...
                    ctx.shadowBlur = 0;
                    ctx.fillText(String(dist), textX, textY);
                }
            }
        }
        ...
        function syncLiveReplayFromServerState(state) {
            ...
            if (nh === 1 && boardEmpty) {
                ...
                if (state.replayOpponentDistanceGrids && state.replayOpponentDistanceGrids.length === 1)
                    ps.liveReplayOpponentDistanceGrids = cloneOpponentDistanceGrids(state.replayOpponentDistanceGrids);
                else if (state.opponentDistanceGrid)
                    ps.liveReplayOpponentDistanceGrids = [cloneOpponentDistanceGrid(state.opponentDistanceGrid)];
                else {
                    const z = Array(ps.BOARD_SIZE).fill().map(() => Array(ps.BOARD_SIZE).fill(null));
                    ps.liveReplayOpponentDistanceGrids = [z];
                }
                return;
            }
            ...
            if (Array.isArray(rvb) && rvb.length === nh) {
                ...
                if (Array.isArray(state.replayOpponentDistanceGrids) && state.replayOpponentDistanceGrids.length === nh)
                    ...
                    ps.liveReplayOpponentDistanceGrids = [];
                return;
            }
        ...
        const variantOpts = {
            ...
            syncState(state) {
                ...
                if (state.boardSize && state.boardSize !== ps.BOARD_SIZE) {
                    ...
                    ps.liveReplayInvisibleTints = [];
                    ps.liveReplayOpponentDistanceGrids = [];
                    ps.serverOpponentDistanceGrid = null;
                    ps.liveViewStep = 0;
                ...
                if (state.opponentDistanceGrid)
                    ps.serverOpponentDistanceGrid = cloneOpponentDistanceGrid(state.opponentDistanceGrid);
                if (state.replayOpponentDistanceGrids && state.replayOpponentDistanceGrids.length)
                    ps.liveReplayOpponentDistanceGrids = cloneOpponentDistanceGrids(state.replayOpponentDistanceGrids);
