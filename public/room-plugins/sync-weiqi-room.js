...
window.RoomPlugins['sync-weiqi'] = {
    shell: {
        "title": "同步围棋",
        "rulesHtml": "基本规则同围棋，贴目与路数规则与标准围棋一致。<br /><br />每回合双方同时落子（或虚着），同时亮出。<br />• 若落子位置不同，则按围棋规则同时生效并提子。<br />• 若双方落在同一空点，该点变为洞。洞不可落子、不提供气<br />• 若双方落子后局面与历史中某一成功局面全同（含洞位置），则本回合两手均无效，双方所选点均变为洞。<br /><br />双方连续虚着时对局结束。",
