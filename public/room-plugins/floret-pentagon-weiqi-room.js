...
window.RoomPlugins["floret-pentagon-weiqi"] = {
    ...
    mount: function (ctx) {
        ...
        var recordDownloadPrefix = config.recordDownloadPrefix != null ? config.recordDownloadPrefix : "花形五角围棋";
        var minLib = config.minLib != null ? config.minLib : 1;
        var standardWeiqiMatchTime = config.standardWeiqiMatchTime != null ? config.standardWeiqiMatchTime : true;


        (function () {
// ======================== 棋盘生成（与后端 generateFloretPentBoardData 一致：格矢 1.5√7×1.5√21；i,j 范围见函数内注释） ========================
        ...
        function generateFloretPentBoard(n) {
...
const scoreTitle = document.getElementById('scoreTitle');
        ...
        /** 花形五角棋盘各路数的总点数（由服务端生成器实测 n=3..13；含 √3 浮点边界无简洁闭式，写死） */
        const FLORET_TOTAL_POINTS = {
            3: 106, 4: 214, 5: 427, 6: 712, 7: 1069, 8: 1375,
            9: 1858, 10: 2413, 11: 3040, 12: 3544, 13: 4297
        };
        function refreshFloretKomiInfo() {
            const el = document.getElementById('komiInfo');
            if (!el) return;
            const t = FLORET_TOTAL_POINTS[BOARD_SIZE];
            if (t == null) el.textContent = '黑贴白' + KOMI + '点';
            else QiWeiqiSquarePageRuntime.writeKomiInfoText(el, KOMI, t);
        }
        refreshFloretKomiInfo();
...
komiInfo,
            ...
            initBoardArray: initFloretBoardArray,
            exitReplayMode,
            clearEstimate,
            hideScoreConfirm,
            showEstimate,
