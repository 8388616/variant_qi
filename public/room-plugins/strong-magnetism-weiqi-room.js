...
window.RoomPlugins['strong-magnetism-weiqi'] = {
    shell: {
        "title": "强磁性围棋",
        "rulesHtml": "基本规则同围棋。<br /><br />棋子具有强磁性。落子时，上下左右四个方向上的所有棋子都会受到磁力影响而移动：己方棋子远离落子点，对方棋子靠近落子点，直至遇到阻碍。<br /><br />移动时先移动己方棋子（同一方向上由远到近），再移动对方棋子（由近到远）；同一方向可重复多轮，直到一整轮内没有任何棋子能移动为止。<br />",
        "defaultKomiText": "黑贴白3.25点",
        "boardSizeMin": 7,
        "boardSizeMax": 21,
        "defaultBoardSize": 19,
        "minLib": 1,
        "recordDownloadPrefix": "强磁性围棋",
    ...
    mount: function (ctx) {
        ...
        var recordDownloadPrefix = config.recordDownloadPrefix != null ? config.recordDownloadPrefix : "强磁性围棋";
...
const scoreTitle = document.getElementById('scoreTitle');
        ...
        function magneticApplyOneRoundStrongClient(board, row, col, playerVal, boardSize) {
        ...
        function magneticApplyMoveStrongClient(board, row, col, playerVal, boardSize) {
            let guard = 0;
            const maxRounds = boardSize * boardSize * boardSize * 8 + 1000;
            while (magneticApplyOneRoundStrongClient(board, row, col, playerVal, boardSize)) {
                if (++guard > maxRounds) break;
            }
            return board;
        }

        function strongMagneticTryPlaceStone(boardBefore, row, col, playerVal) {
            ...
            magneticApplyMoveStrongClient(newBoard, row, col, playerVal, boardSize);
        ...
        function magneticApplyOneRoundStrongClientWithNum(board, numGrid, row, col, playerVal, boardSize) {
            const dirs = [[0, -1], [0, 1], [-1, 0], [1, 0]];
            const ownList = [];
            const oppList = [];
            for (let dirIdx = 0; dirIdx < dirs.length; dirIdx++) {
        ...
        function magneticApplyMoveStrongClientWithNum(board, numGrid, row, col, playerVal, boardSize) {
            let guard = 0;
            const maxRounds = boardSize * boardSize * boardSize * 8 + 1000;
            while (magneticApplyOneRoundStrongClientWithNum(board, numGrid, row, col, playerVal, boardSize)) {
                if (++guard > maxRounds) break;
            }
            return board;
        }

        function strongMagneticTryPlaceStoneWithNum(boardBefore, numGridBefore, row, col, playerVal, moveNum) {
            const boardSize = ps.BOARD_SIZE;
            if (boardBefore[row][col] !== 0) return null;
            const newBoard = QiSquareWeiqiCanvas.deepCopyBoard(boardBefore);
            ...
            magneticApplyMoveStrongClientWithNum(newBoard, newNum, row, col, playerVal, boardSize);
        ...
        function computeStrongMagneticStoneNumbers() {
            ...
            const applySeq = (moves, upTo) => {
                ...
                for (let j = 0; j < lim; j++) {
                    ...
                    const r = strongMagneticTryPlaceStoneWithNum(board, numGrid, mv.row, mv.col, pv, mi);
                    if (r) {
                        board = r.board;
                        numGrid = r.numGrid;
                    }
            ...
            if (ps.replayMode && ps.tryPlayMode) {
                ...
                for (let i = 1; i <= ps.tryPlayStep; i++) {
                    ...
                    if (markers && markers.length > 0) {
                        const m = markers[0];
                        const r = strongMagneticTryPlaceStoneWithNum(board, numGrid, m.row, m.col, m.color, i);
                        if (r) {
                            board = r.board;
                            numGrid = r.numGrid;
                        }
        ...
        function strongMagneticDrawBoard() {
            ...
            if (ps.showMoveNumbers) {
                const nums = computeStrongMagneticStoneNumbers();
