...
const scoreTitle = document.getElementById('scoreTitle');
        ...
        function tryPlaceShape(boardBefore, shapeIdx, rot, flip, refRow, refCol, playerVal) {
            ...
            const owners = SHAPE_STONE_OWNERS;
        ...
        function tryPlaceStonesAt(boardBefore, stoneCoords, playerVal, stoneOwners) {
        ...
        function tryPlaceSingleRusStone(boardBefore, row, col, playerVal) {
            ...
            if (countGroupLiberties(newBoard, row, col) === 0)
        ...
        function isUserBoardMarkVisibleAt(r, c) {
            ...
            if (r < 0 || r >= ps.BOARD_SIZE || c < 0 || c >= ps.BOARD_SIZE) return false;
            if (ps.board[r][c] !== 0) return false;
            const canPreview = ps.tryPlayMode && ps.replayMode ? !ps.waitingScoreConfirm
                : (!ps.gameOver && (ps.isMyTurn || (ps.tryPlayMode && ps.replayMode)));
            if (canPreview) {
                        ...
                        if (coords) {
                            for (let idx = 0; idx < coords.length; idx++) {
                                ...
                                if (ps.board[r][c] === 0) {
                                    ...
                                    ctx.beginPath();
                                    ctx.arc(cx, cy, radius, 0, 2 * Math.PI);
                                    ctx.fillStyle = SHAPE_STONE_OWNERS[idx] === 'opp' ? oppColor : selfColor;
                                    ctx.fill();
                                } else {
        ...
        function rebuildCompoundLive(moveCoords) {
            ...
            for (const move of (moveCoords || [])) {
                ...
                if (move.type === 'move' && move.stones) {
                    ...
                    liveReplayMarkers.push(move.stones.map(([r, c], j) => ({
                        row: r, col: c,
            ...
            for (const move of (data.moves || [])) {
                ...
                if (move.type === 'move' && move.stones) {
                    const owners = move.stoneOwners && move.stoneOwners.length === 3
                        ? move.stoneOwners
                        : ['self', 'opp', 'self'];
                    const nb = tryPlaceStonesAt(curBoard, move.stones.map(([r, c]) => [r, c]), playerVal, owners);
                    if (nb) curBoard = nb;
                    replayBoards.push(deepCopyBoard(curBoard));
                    replayMarkers.push(move.stones.map(([r, c], j) => ({
                        row: r, col: c,
                        color: owners[j] === 'opp' ? (3 - playerVal) : playerVal
                    })));
                    let si = move.shapeIndex;
                    if (typeof si !== 'number' || si < 0) {
        ...
        function updateShapeColors() {
            ...
            document.querySelectorAll('.piece-btn .stone').forEach(stone => {
                const owner = stone.getAttribute('data-owner');
                const selfTone = isWhite ? 'white' : 'black';
                const oppTone = isWhite ? 'black' : 'white';
                stone.setAttribute('data-tone', owner === 'self' ? selfTone : oppTone);
            ...
            SHAPE_LIST.forEach((shape, idx) => {
                ...
                for (let r = -1; r <= 1; r++) {
                    for (let c = -1; c <= 1; c++) {
                        ...
                        const posIndex = shape.findIndex(([rr, cc]) => rr === r && cc === c);
                        if (posIndex !== -1) {
                            const stone = document.createElement('div');
                            stone.className = 'stone';
                            stone.setAttribute('data-owner', posIndex === 1 ? 'opponent' : 'self');
        ...
        const page = QiWeiqiSquarePageRuntime.create(ps, domPage, {
            ...
            tryPlayMove(row, col) {
                ...
                const markers = coords.map(([r, c], i) => ({
                    row: r, col: c,
                    color: SHAPE_STONE_OWNERS[i] === 'opp' ? (3 - playerVal) : playerVal
                }));
