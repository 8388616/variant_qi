...
const PIECE_CHAR = {
    wk: '♚', wq: '♛', wr: '♜', wn: '♞', wb: '♝', wp: '♟', wf: '♛',
    bk: '♚', bq: '♛', br: '♜', bn: '♞', bb: '♝', bp: '♟', bf: '♛'
};
...
return {
    ...
    metaFrom
        ...
        function drawBoard() {
            ...
            // 棋格不填充颜色，直接用棋盘木色背景；只画格线（颜色粗细与中国象棋相同：#3a281c、1.8）
            for (let r = 0; r < R.BOARD_H; r++) {
                for (let c = 0; c < R.BOARD_W; c++) {
            ...
            for (let r = 0; r < R.BOARD_H; r++) {
                for (let c = 0; c < R.BOARD_W; c++) {
                    ...
                    // 象与士倒置显示（旋转 180°）
                    const upsideDown = piece === 'wb' || piece === 'bb' || piece === 'wf' || piece === 'bf';
                    if (upsideDown) {
                        ctx2d.save();
                        ctx2d.translate(x, y);
