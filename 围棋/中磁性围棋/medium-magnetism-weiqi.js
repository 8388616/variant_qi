...
/** 客户端「显示序号」按 moveCoords 与磁性规则重放，使序号随子移动（见 中磁性围棋.html）。 */
class MediumMagneticWeiqiRoom extends QiTwoPlayerRoomBase
{
    ...
    importRecord(data, requesterWs) {
        if (!data || data.gameId !== 'medium-magnetism-weiqi') {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要中磁性围棋棋谱）。' }));
            return;
        }
        ...
        const moves = rawMoves.map(MediumMagneticWeiqiRoom.parseMove);
        ...
        if (data.result || data.resultText) {
            ...
            this.winner = MediumMagneticWeiqiRoom.parseResultTextToWinner(importedResultText);
...
module.exports = {
    initRoom(room) {
        room.gameLogic = new MediumMagneticWeiqiRoom(room);
