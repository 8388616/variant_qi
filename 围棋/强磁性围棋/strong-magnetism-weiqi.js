...
class StrongMagneticWeiqiRoom extends QiTwoPlayerRoomBase
{
    ...
    /**
     * 强磁性：与中磁性同序（先己方由远到近，再对方由近到远），但每一「轮」每人最多一格；
     * 重复多轮直到一整轮内没有任何棋子能移动为止。
     */
    ...
    importRecord(data, requesterWs) {
        if (!data || data.gameId !== 'strong-magnetism-weiqi') {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要强磁性围棋棋谱）' }));
            return;
        }
        ...
        const moves = rawMoves.map(StrongMagneticWeiqiRoom.parseMove);
        ...
        if (data.result || data.resultText) {
            ...
            this.winner = StrongMagneticWeiqiRoom.parseResultTextToWinner(importedResultText);
...
module.exports = {
    initRoom(room) {
        room.gameLogic = new StrongMagneticWeiqiRoom(room);
