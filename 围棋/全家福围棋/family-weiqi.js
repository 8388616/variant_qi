...
/**
 * 形势判断：洞/雷/中立子仍不可作为 BFS 落脚点；桥不可落脚但可沿四方向直穿连续桥格，只计一步。
 */
...
class FamilyWeiqiRoom extends QiTwoPlayerRoomBase {
    ...
    generateFamilyBoard() {
        ...
        for (let r = 0; r < this.boardSize; r++) {
            for (let c = 0; c < this.boardSize; c++) {
                const x = Math.random();
                if (x < 0.05) board[r][c] = HOLE;
                else if (x < 0.1) board[r][c] = NEUTRAL;
                else if (x < 0.15) board[r][c] = BRIDGE;
                else if (x < 0.2) board[r][c] = MINE;
            }
        }
        return board;
    }
    ...
    {
        if (!this.inBounds(row, col) || boardBefore[row][col] !== 0)
            return null;
        ...
        for (const [nr, nc] of this.getBridgeNeighbors(board, row, col)) {
            const v = board[nr][nc];
            if ((v === 3 - playerVal || v === NEUTRAL) && !this.hasLiberty(board, nr, nc)) 
            {
                const key = `${v}:${nr},${nc}`;
                if (!seen.has(key))
                {
        ...
        for (const stone of toCapture) 
        {
            if (board[stone.row][stone.col] === stone.color)
                this.removeGroup(board, stone.row, stone.col, stone.color);
        }
    ...
    removeDeadAndDyingForFamily(boardSrc) {
        ...
        while (changed) {
            ...
            for (let r = 0; r < this.boardSize; r++) {
                for (let c = 0; c < this.boardSize; c++) {
                    const v = board[r][c];
                    if ((v === 1 || v === 2 || v === NEUTRAL) && !this.hasLiberty(board, r, c)) {
    ...
    computeLead() {
        const KOMI = 4.75;
        const liveBoard = this.removeDeadAndDyingForFamily(this.board);
        const territory = assignTerritoryBridgeAware(liveBoard, this.boardSize, {
            isPassable: (v) => v !== HOLE && v !== BRIDGE && v !== MINE && v !== NEUTRAL
        });
            ...
            case 'editBoard':
                ...
                for (let r = 0; r < this.boardSize; r++) {
                    for (let c = 0; c < this.boardSize; c++) {
                        ...
                        if (val !== HOLE && val !== BRIDGE && val !== MINE && val !== NEUTRAL && val !== 0 && val !== 1 && val !== 2) {
    ...
    importRecord(data, requesterWs) {
        if (!data || (data.gameId && data.gameId !== 'family-weiqi' && data.game !== '全家福围棋')) {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要全家福围棋棋谱）。' }));
            return;
        }
        if (data.boardSize && data.boardSize >= 7 && data.boardSize <= 21) this.boardSize = data.boardSize;
        this.resetToEmpty();
        ...
        this.historyBoardSet.clear();
        this.historyBoardSet.add(this.boardToString(this.board));
        const moves = (data.moves || []).map(m => FamilyWeiqiRoom.parseMove(m));
        for (let i = 0; i < moves.length; i++) {
            ...
            if (move.type === 'move') {
                ...
                if (this.board[row][col] === MINE) this.board[row][col] = 0;
                this.historyBoards.push(this.copyBoard(this.board));
                this.historyMarkers.push(this.copyMarkers(this.lastMoveMarkers));
                this.moveHistory.push(move.player);
                this.moveCoords.push({ type: 'mineHit', player: move.player, row, col });
                this.currentPlayer = 3 - this.currentPlayer;
                this.passCounter = 0;
                this.lastMoveMarkers = [{ row, col, color: playerVal }];
        ...
        if ((data.result || data.resultText) && !this.gameOver) {
            ...
            this.winner = FamilyWeiqiRoom.parseResultTextToWinner(importedResultText);
