...
// 白马 l(4,9) 邻格 = h(4,6)(wb?)、r(4,9)(wq?)、h(6,6)(wb?)、l(6,9)(wk?)——空盘上邻格 = h(4,6) r(4,9) h(6,6) l(6,9)
// 马先邻后斜：h(4,6) 的斜走 = h(2,6) h(6,6)——h(6,6) 空 ✓
check('马先邻后斜（h(4,6)→h(6,6) 邻格起）', wn9.some(m => m.to === findId('h', 6, 6)));
// r(4,9) 的斜走 = r(5,6) r(3,12)？r(4,9) 斜走 = r(5,6)（左下）、r(3,12) 不在——r(5,6) ✓
check('马先邻后斜（r(4,9)→r(5,6)）', wn9.some(m => m.to === findId('r', 5, 6)));
// 先斜后邻：l(4,9) 斜走 = l(5,12)？l(4,9) 斜走 = l(5,12)（右下不在）、l(3,6)（左上 ✓）——l(3,6) 的邻格 = h(2,6)？l(3,6) 邻格 = r(3,6)（对格）、h(1,3)？——检查 h(2,6) 是否可达
check('马先斜后邻（l(3,6)→h(2,6)）', wn9.some(m => m.to === findId('h', 2, 6)));
