...
class CaptureQiziqiRoom extends QiTwoPlayerRoomBase {
    constructor(room) {
        super(room);
        this.BOARD_SIZE = 17;
    ...
    checkSevenInRow(board, row, col, colorVal) {
    ...
    importRecord(data, requesterWs) {
        if (!data || data.gameId !== 'capture-qiziqi') {
            requesterWs.send(JSON.stringify({ type: 'error', message: '棋谱格式不匹配（需要提子七子棋棋谱）。' }));
            return;
        }
        const newSize = data.boardSize || 17;
        if (!Number.isInteger(newSize) || newSize < 7 || newSize > 21) {
        ...
        for (let i = 0; i < rawMoves.length; i++) {
            ...
            if (this.checkSevenInRow(boardAfterPlacement, row, col, playerVal)) {
...
module.exports = {
    initRoom(room) {
        room.gameLogic = new CaptureQiziqiRoom(room);
        room.maxPlayers = 2;
    }
};
